    ...
    for mode in MODES {
        group.bench_function(mode, |b| {
            b.iter_batched(
                ...
                // Hand the player back instead of letting it drop here: it owns
                // a full frame buffer, and freeing that inside the timed region
                // measures the allocator returning pages to the OS (milliseconds,
                // roughly once per batch) instead of the tick. Criterion drops
                // returned values outside the measurement.
                |(mut player, tick_at)| {
                    let changed = black_box(player.advance(tick_at));
                    (player, changed)
                },
