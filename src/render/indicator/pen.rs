...
mod tests {
    ...
    /// coverage, so those chars must fall back to a drawable .notdef rather
    ...
    /// Measuring the fallback is not enough — the draw path resolves the glyph
    /// separately, so rasterize an uncovered char and require ink on the
    /// pixmap. The empty string is the negative control: if this counted ink
    /// either way, the test would pass on a broken fallback.
    #[test]
    fn uncovered_char_rasterizes_visible_ink() {
        fn ink(text: &str) -> usize {
            let mut pix = Pixmap::new(64, 64).unwrap();
            let mut pen = Pen::new(&mut pix);
            pen.draw_text_centered(32.0, 32.0, 20.0, text, (1.0, 1.0, 1.0));
