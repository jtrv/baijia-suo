...
fn rand_sign(rng: &mut impl Rng) -> i64 {
    if rng.random::<bool>() { 1 } else { -1 }
}

fn nrand(rng: &mut impl Rng, v: i64) -> i64 {
...
fn make_blob(maxx: i32, maxy: i32, size: i32, rng: &mut impl Rng) -> Blob {
...
fn throb_blob(b: &mut Blob, rng: &mut impl Rng) {
...
fn move_blob(b: &mut Blob, maxx: i32, maxy: i32, rng: &mut impl Rng) {
    let maxx = (maxx as i64) * SCALE;
    let maxy = (maxy as i64) * SCALE;
