...
fn setup_multi_strike(rng: &mut impl RngExt) -> usize {
    multi_strike_for(rng.random_range(0..100))
}

fn flash_duration(total_duration: i32, rng: &mut impl RngExt) -> (i32, i32) {
...
fn generate(a: XPoint, b: XPoint, iter: u32, verts: &mut [XPoint], vert_index: &mut usize, rng: &mut impl RngExt) {
...
fn create_fork(f: &mut Fork, start: XPoint, end: XPoint, level: usize, rng: &mut impl RngExt) {
    let mut tmp = 1;
...
fn random_storm(bolts: &mut [LightningBolt], scr_width: u32, scr_height: u32, multi_strike: usize, rng: &mut impl RngExt) {
...
fn wiggle_line(points: &mut [XPoint], amount: i32, rng: &mut impl RngExt) {
    if amount <= 0 {
        return;
    }
...
fn wiggle_bolt(bolt: &mut LightningBolt, rng: &mut impl RngExt) {
...
fn update_bolt(bolt: &mut LightningBolt, time_now: i32, rng: &mut impl RngExt) {
