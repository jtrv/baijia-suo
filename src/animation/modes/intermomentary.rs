...
fn frand(rng: &mut impl Rng, up: f32) -> f32 {
    rng.random::<f32>() * up
}
...
impl Intermomentary {
    fn make_disc(&mut self, rng: &mut impl Rng, x: f32, y: f32, vx: f32, vy: f32, r: f32) {
    ...
    fn render_disc(&mut self, rng: &mut impl Rng, dnum: usize) {
